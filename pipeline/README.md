...
★★★ **R017: 생성 구조를 갈아엎었다 — `소분류 → 소재(seed) → 소재당 문제 1개`.**
파일럿 200건을 만들어 측정했다. ★ DB 적재는 건우 승인 대기다.
...
```
  ...
  [★★ R017부터]
     ★ 소분류 + 금지 목록 → **소재 목록**(프롬프트 A / Opus)
        → **소재당 문제 1개**(프롬프트 B / Opus)
        → 규칙 검사(API 0회) → 정답 정규화로 **후보 추리기**
        → ★ 후보만 **LLM 중복 판정**(Gemini — 생성과 다른 모델)
        → 역검증(Sonnet, Q-81) → 검수 → 적재
```
...
### ★ R017 파일럿 실측 (소분류 20개 × 소재 10개 = 200건)

| 지표 | 값 | 비교 |
|------|-----|------|
| 소재 요청 대비 생성 | 200 / 200 (100%) | 미달 소분류 0개 |
| 문제 생성 성공 | 200 / 200 (100%) | `ok:false` 0건 |
| ★★ 생성분 안 정답 중복 | **★ 0쌍** (19,900쌍 중) | R012 비교 실험 약 38% |
| 기존 306건과의 후보 | 63,200쌍 → **5쌍** (0.008%) | R011 은 0.036% |
| ★ 그중 실제 중복 | **1쌍** | ★★ 서로 **다른 소분류**였다 |
| `acceptedAnswers` 빈 배열 | 178 / 200 (89.0%) | 설계 의도와 맞는다 |
| 알 가치 1~2 | 0건 (평균 3.93) | |
| 난이도 4 이상 | 58건 (29.0%, 평균 3.04) | g3 시절 평균 2.05~2.38 |

★★ **한계를 그대로 적는다.** 200건이 소분류 20개에만 들어 있고, 그 20개는 서로 겹치지 않게
일부러 골랐다. ★ 소분류 **사이** 중복을 재는 시험으로는 약하다. 297개로 넓혀 다시 재야 한다.
...
| ★★ `pipeline/prompts/seed-v1.md` | **소재 목록 생성 프롬프트.** 건우 확정본. 버전을 파일명으로 고정한다 (D-079) |
| ★★ `pipeline/prompts/question-v1.md` | **문제 생성 프롬프트.** 소재 하나 → 문제 하나 |
| ★★ `pipeline/lib/seedstore.mjs` | **소재 저장소.** 중분류당 한 파일 + 금지 목록 조립 (D-080) |
| ★ `pipeline/lib/subid.mjs` | 소분류 식별자 `"kr-language/1"`. ★ 이름을 파일명으로 쓰지 않는다 |
| ★ `scripts/seeds-init-from-db.mjs` | 기존 정답 → `usedAnswers` 초기값 (★ DB 읽기만) |
| ★ `scripts/seeds-banlist.mjs` | ★★ 프롬프트 A 입력 조립. 금지 목록이 들어갔는지 눈으로 본다 |
| ★ `scripts/seeds-ingest.mjs` | 배치 반영 + 상태 기록 (★ 중단·재개) |
| ★ `scripts/r017-check.mjs` | 규칙 검사 + 중복 후보 추리기 (★ API 0회) |
| ★★ `scripts/r017-dupe-judge.mjs` | 후보만 Gemini 중복 판정 (★ 생성 모델과 달라야 한다) |
| ★ `scripts/r017-stats.mjs` | 파일럿 집계 |
| `pipeline/src/gen-prompt.ts` | ★★ **R017에서 비활성화.** 프롬프트 `g3` (카테고리→문제 여러 개). 지우지 않았다 (D-077) |
...
| `scripts/pipeline-bulk.mjs` | ★★ **R017에서 비활성화** (`--legacy` 필요). 옛 주 실행 명령 (D-049) |
...
| `g3` → ★★ **`seed-v1` + `question-v1`** (R017) | ★★★ **구조 자체가 바뀌었다.** 프롬프트 개정이 아니다 — `카테고리 → 문제 여러 개` 를 `소분류 → 소재 → 소재당 문제 1개` 로 나눴다. ★ 근거: 다양성의 병목이 문장 표현이 아니라 **소재 선택**이라는 실측(겹침 38%). ★ 그리고 프롬프트 정본을 코드 문자열에서 **파일**로 옮겼다 (D-079) |
