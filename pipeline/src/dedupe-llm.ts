...
//
// ★★★ R019: 판정 기준을 **"그 문제를 맞히게 해주는 지식이 같은가"** 로 못박았다 (Q-91 확정).
//   ★ 근거: R018 에서 생성 세션과 판정자가 11쌍 중 4쌍 갈렸고 네 쌍 모두 판정자가 same 이었다.
//     ★ 생성 프롬프트(seed-v2)에 **같은 문장**을 넣었다 — 두 쪽이 같은 기준을 보게 하는 것이 목적이다.
//
// ★★ 왜 LLM 인가
...
★★★ 판정 기준 — **그 문제를 맞히게 해주는 지식이 같은가**  (Q-91 확정)

  ★★ 문장이 닮았는지가 아니다. **묻는 사실이 다른지도 아니다.**
    묻는 사실은 얼마든지 다르게 쓸 수 있다.
  ★★★ **한 사람이 지식 하나를 알고 있을 때 두 문제를 모두 바로 맞히는가**를 보라.
    그렇다면 같은 문제다. 문장이 아무리 달라도 그렇다.

  ★★ **"대상이 같은가" 로 판단하지 마라.** 대상이 같아도 지식이 다를 수 있다 —
    "금메달의 몸체를 이루는 금속은?" 과 "원소 기호 Ag 는?" 은 둘 다 답이 '은' 이고
    대상도 '은' 이지만, "금메달은 은에 도금한 것이다" 와 "Ag 는 은이다" 는 별개의 지식이다.
    ★ 한쪽을 아는 사람이 다른 쪽을 맞힌다는 보장이 없다. → **different**
  ...
  "same" (같은 문제)
    ...
    예: "9개의 교향곡을 남긴 청각 장애 작곡가는?" 과
        ...
        → 둘 다 베토벤을 묻는다. **same**
    ★★ 예: "오륜기 도안을 그린 사람은?" 과 "근대 올림픽을 되살린 프랑스 교육자는?"
        → 묻는 사실은 다르다. ★ 그런데 "쿠베르탱이 근대 올림픽을 만들었다" 는
          지식 하나를 아는 사람은 **둘 다 바로 맞힌다.** **same**

  "different" (다른 문제)
    ★★ 정답이 같더라도, 그리고 **대상이 같더라도**, 맞히게 해주는 지식이 다르다.
    예: "한국어 '빵'은 어느 나라 언어에서 왔는가?" (어원 지식) 과
        "브라질의 공용어는?" (지리 지식)
        → 둘 다 답이 포르투갈어지만 **different**
    예: "올림픽 금메달의 몸체를 이루는 금속은?" 과 "원소 기호 Ag 로 적는 금속은?"
        → 둘 다 답이 "은" 이고 대상도 같지만 **지식이 별개**다. **different**
    예: "1950년 대회에서 브라질을 꺾은 나라는?" 과 "1930년 제1회 월드컵 개최국은?"
        → 둘 다 답이 "우루과이" 지만 아는 것이 별개다. **different**
    예: "주파수의 단위는?" 과 "전자기파를 실험으로 확인한 물리학자는?"
        → 둘 다 답이 "헤르츠" 지만 지식이 별개다. **different**
